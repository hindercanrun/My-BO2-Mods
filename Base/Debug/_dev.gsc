...
init()
{
	...
	if( getdvar( "debug_corner" ) == "" )
	{
		SetDvar( "debug_corner", "off" );
	}
	else if	( getdvar( "debug_corner" ) == "on" )
	{
		debug_corner();
	}
...
//T6_Dtodo	thread addTestClients();
//T6_Dtodo	thread addEnemyHeli();
//T6_Dtodo	thread addEnemyU2();
//T6_Dtodo	thread addTestCarePackage();
//T6_Dtodo	thread removeTestClients();
//T6_Dtodo	thread watch_botsdvars();
//T6_Dtodo	thread devHeliPathDebugDraw();
//T6_Dtodo	thread devStrafeRunPathDebugDraw();
//T6_Dtodo	thread maps\mp\gametypes\_dev_class::dev_cac_init();
//T6_Dtodo	thread maps\mp\gametypes\_globallogic_score::setPlayerMomentumDebug();
	...
	// SRS 3/19/08: engagement distance debug dvar toggle watcher
//T6_Dtodo	thread engagement_distance_debug_toggle();
	
	// give equipment through devgui
//T6_Dtodo	thread equipment_dev_gui();
	
	// give grenades through devgui
//T6_Dtodo	thread grenade_dev_gui();
...
//T6_Dtodo		player thread watchAttachmentChange();
	}
}
	...
	while( 1 )
	{
		if( GetDvar( "chain" ) == "1" )
		{
			for( i=0;i<fnodes.size;i++ )
			{
				...
				{
//T6_Dtodo					print3d( fnodes[ i ].origin, "yo", ( 0.2, 0.8, 0.5 ), 0.45 );
					/*
					count++;
					if( count > 25 )
					{
			...
			for( i=0;i<friends.size;i++ )
			{
				...
				if( isdefined( node ) )
				{
//T6_Dtodo					line( friends[ i ].origin +( 0, 0, 35 ), node.origin, ( 0.2, 0.5, 0.8 ), 0.5 );
				}
			}
...
debug_enemyPosProc()
{
	...
	for( ;; )
	{
		...
		{
//T6_Dtodo			line( self.origin +( 0, 0, 70 ), self.enemy.origin +( 0, 0, 70 ), ( 0.8, 0.2, 0.0 ), 0.5 );
		}
...
//T6_Dtodo		line( self.origin +( 0, 0, 70 ), pos, ( 0.9, 0.5, 0.3 ), 0.5 );
	}
}
...
//			continue;
		...
		{
//T6_Dtodo			line( guy.origin +( 0, 0, 65 ), guy.lastEnemySightPos, ( 1, 0, 1 ), 0.5 );
		}
...
//T6_Dtodo			draw_arrow( nodeOffset, guy.goodShootPos, color );
		}
//		break;	
	}
	if( 1 )
	...
	{
//T6_Dtodo		line( guy.origin +( 0, 0, 70 ), guy.enemy.origin +( 0, 0, 70 ), ( 0.6, 0.2, 0.2 ), 0.5 );
	}

	if( isdefined( guy.lastEnemySightPos ) )
	{
//T6_Dtodo		line( guy.origin +( 0, 0, 65 ), guy.lastEnemySightPos, ( 0, 0, 1 ), 0.5 );
	}

	if( isalive( guy.goodEnemy ) )
	{
//T6_Dtodo		line( guy.origin +( 0, 0, 50 ), guy.goodEnemy.origin, ( 1, 0, 0 ), 0.5 );
	}
...
//T6_Dtodo	line( guy.origin +( 0, 0, 55 ), pos, ( 0.2, 0.2, 0.6 ), 0.5 );

	if( guy.goodShootPosValid )
	{
//T6_Dtodo		line( guy.origin +( 0, 0, 45 ), guy.goodShootPos, ( 0.2, 0.6, 0.2 ), 0.5 );
	}
}
...
drawEntTag( num )
{
	...
	for( i=0;i<ai.size;i++ )
	{
//T6_Dtodo		if( ai[ i ] getentnum() != num )
//T6_Dtodo		{
//T6_Dtodo			continue;
//T6_Dtodo		}
...
//T6_Dtodo	line( org, org + forwardFar, red, 0.9 );
//T6_Dtodo	line( org + forwardFar, org + forwardClose + rightdraw, red, 0.9 );
//T6_Dtodo	line( org + forwardFar, org + forwardClose + leftdraw, red, 0.9 );
//T6_Dtodo
//T6_Dtodo	line( org, org + right, blue, 0.9 );
//T6_Dtodo	line( org, org + up, green, 0.9 );
}
...
//T6_Dtodo		spawn thread debugorigin();
...
createLine( org )
{
	for( ;; )
	{
//T6_Dtodo		line( org +( 0, 0, 35 ), org, ( 0.2, 0.5, 0.8 ), 0.5 );
		wait( 0.05 );
	}
}
...
createLineConstantly( ent )
{
	...
	for( ;; )
	{
//T6_Dtodo		line( org +( 0, 0, 35 ), org, ( 1.0, 0.2, 0.1 ), 0.5 );
		wait( 0.05 );
	}
}
...
debugMisstime()
{
	...
	for( ;; )
	{
		...
		{
//T6_Dtodo			print3d( self gettagorigin( "TAG_EYE" ) +( 0, 0, 15 ), "hit", ( 0.3, 1, 1 ), 1 );
		}
		else
		{
//T6_Dtodo			print3d( self gettagorigin( "TAG_EYE" ) +( 0, 0, 15 ), self.a.misstime/20, ( 0.3, 1, 1 ), 1 );
		}
		wait( 0.05 );
	}
}
...
//T6_Dtodo		}
		...
		// JesseS (3/20/2007): removed level.player, added players[0]
		players = get_players();			
//T6_Dtodo		line (players[0].origin, ai[i].origin, (0.2,0.3,1.0));
		return;
	}
}
...
debugDvars()
{
	...
	if( getdvar( "debug_lookangle" ) == "" )
	{
		SetDvar( "debug_lookangle", "off" );
	}

	if( getdvar( "debug_grenademiss" ) == "" )
	{
		SetDvar( "debug_grenademiss", "off" );
	}

	if( getdvar( "debug_enemypos" ) == "" )
	{
		SetDvar( "debug_enemypos", "-1" );
	}
		
	if( getdvar( "debug_dotshow" ) == "" )
	{
		SetDvar( "debug_dotshow", "-1" );
	}
		
	if( getdvar( "debug_stopenemypos" ) == "" )
	{
		SetDvar( "debug_stopenemypos", "-1" );
	}

	if( getdvar( "debug_replayenemypos" ) == "" )
	{
	...
	if( getdvar( "debug_dynamic_ai_spawning" ) == "" )
	{
		SetDvar( "debug_dynamic_ai_spawning", "0" );
	}
		
	level.last_threat_debug = -23430;
	if( getdvar( "debug_threat" ) == "" )
	{
	...
	for( ;; )
	{
		...
		if( getdvar( "debug_tag" ) != "" )
		{
			thread viewTag( "ai", getdvar( "debug_tag" ) );
			if( getdvarInt( "debug_enttag" ) > 0 )
			{
				thread drawEntTag( getdvarInt( "debug_enttag" ) );
			}
		}

		if( getdvar( "debug_goalradius" ) == "on" )
		{
			level thread debug_goalradius();
		}
		
		if( getdvar( "debug_maxvisibledist" ) == "on" )
		{
			level thread debug_maxvisibledist();
		}
		
		if( getdvar( "debug_health" ) == "on" )
		{
			level thread debug_health();
		}
		
		if( getdvar( "debug_engagedist" ) == "on" )
		{
		...
		if( getdvar( "debug_vehicletag" ) != "" )
		{
			thread viewTag( "vehicle", getdvar( "debug_vehicletag" ) );
		}
		...
		if( getdvar( "debug_vehiclesittags" ) != "off" )
		{
			thread debug_vehiclesittags();
		}

		if( getdvar( "debug_replayenemypos" ) == "on" )
		{
		...
		if( getdvar( "debug_threat" ) != "-1" && getdvar( "debug_threat" ) != "" )
		{
			debugThreat();
		}

		level.debug_badpath = getdvar( "debug_badpath" ) == "on";
		...
		if( getdvar( "debug_dynamic_ai_spawning" ) == "1" && (!isdefined(level.spawn_anywhere_active) || level.spawn_anywhere_active == false) )
		{
			level.spawn_anywhere_active = true;
			level.spawn_anywhere_friendly = getdvar( "debug_dynamic_ai_spawn_friendly" );
			thread dynamic_ai_spawner();
		}
		else if (getdvar( "debug_dynamic_ai_spawning" ) == "0" && isdefined(level.spawn_anywhere_active) && level.spawn_anywhere_active == true )
		{
		...
		if( isdefined(level.spawn_anywhere_active) && level.spawn_anywhere_active == true && getdvar( "debug_dynamic_ai_spawn_friendly" ) != level.spawn_anywhere_friendly )
		{
			level notify ("kill dynamic spawning");	
			level.spawn_anywhere_active = false;
		}

		if( getdvar( "debug_ai_puppeteer" ) == "1" && (!isdefined(level.ai_puppeteer_active) || level.ai_puppeteer_active == false) )
		{
			level.ai_puppeteer_active = true;
			level notify ("kill ai puppeteer");	
//T6_Dtodo			AddCommand( "noclip" );
			thread ai_puppeteer();
		}
		else if (getdvar( "debug_ai_puppeteer" ) == "0" && isdefined(level.ai_puppeteer_active) && level.ai_puppeteer_active == true )
		{
...
{
//T6_Dtodo	reflection_locs = GetReflectionLocs();
//T6_Dtodo	for ( i = 0; i < reflection_locs.size; i++ )
//T6_Dtodo	{
//T6_Dtodo		level.debug_reflection_objects[i] = spawn( "script_model", reflection_locs[i] );
//T6_Dtodo		level.debug_reflection_objects[i] setmodel( "test_sphere_silver" );
//T6_Dtodo	}
}
...
//T6_Dtodo		line(	level.debug_reflectionobject.origin, 
//T6_Dtodo			getreflectionorigin( level.debug_reflectionobject.origin ),
//T6_Dtodo			(1,0,0),
//T6_Dtodo			true,
//T6_Dtodo			1 );
...
//T6_Dtodo		line( start, trace[ "position" ], ( 0.9, 0.5, 0.8 ), 0.5 );
	}	
}
...
hatmodel()
{
	for( ;; )
	{
		...
		{
//T6_Dtodo			println( " " );
//T6_Dtodo			println( "The following AI have no Hatmodel, so helmets can not pop off on head-shot death:" );
			for( i=0;i<noHat.size;i++ )
			{
//T6_Dtodo				println( "Classname: ", noHat[ i ] );
			}
//T6_Dtodo			println( "To disable hatModel spam, type debug_hatmodel off" );
		}
		wait( 15 );
	}
}
...
//T6_Dtodo	setangles( camera.angles );
//T6_Dtodo	setorigin( camera.origin +( 0, 0, -60 ) );
}
/*
	wait(0.05);
	thread anglescheck();
...
deathspawnerPreview()
{
	...
	for( i=0;i<50;i++ )
	{
		...
		for( p=0;p<array.size;p++ )
		{
			...
			{
//T6_Dtodo				print3d( ent.origin, i + ": " + ent.truecount, ( 0, 0.8, 0.6 ), 5 );
			}
			else
			{
//T6_Dtodo				print3d( ent.origin, i + ": " +  ".", ( 0, 0.8, 0.6 ), 5 );
			}
		}
	}
}
...
//T6_Dtodo			}
		...
		for( ;; )
		{
			...
			const range = 10;
			color =( 0.2, 0.9, 0.8 );
//T6_Dtodo			line( org +( 0, 0, range ), org +( 0, 0, range * -1 ), color, 1.0 );
//T6_Dtodo			line( org +( range, 0, 0 ), org +( range * -1, 0, 0 ), color, 1.0 );
//T6_Dtodo			line( org +( 0, range, 0 ), org +( 0, range * -1, 0 ), color, 1.0 );
...
updateMinimapSetting()
{
	...
	if( minimapheight != level.minimapheight )
	{
		...
		if( minimapheight > 0 )
		{
			...
			{
//T6_Dtodo				println( "^1Error: There are not exactly 2 \"minimap_corner\" entities in the level." );
			}
		}
	}
}
...
drawMiniMapBounds( viewpos, mincorner, maxcorner )
{
	...
	while( 1 )
	{
//T6_Dtodo		line( corner0, corner1 );
//T6_Dtodo		line( corner1, corner2 );
//T6_Dtodo		line( corner2, corner3 );
//T6_Dtodo		line( corner3, corner0 );

		array_ent_thread( chains, maps\_utility::plot_points );

//T6_Dtodo		print3d( toppos, "This Side Up", ( 1, 1, 1 ), 1, textscale );
...
debug_vehiclesittags()
{
	...
	type = "none";
	type = getdvar( "debug_vehiclesittags" );
...
//		if( !isdefined( level.vehicle_aianims[ vehicles[ i ].vehicletype ] )  || vehicles[ i ].vehicletype != type )
		...
		for( j=0;j<anims.size;j++ )
		{
			...
			if( isdefined( anims[ j ].sittag ) )
			{
				...
				{
//T6_Dtodo					print3d( org+( 0, 0, 16 ), anims[ j ].sittag, ( 1, 1, 1 ), 1, 1 );
				}
			}
		}
	}
...
//T6_Dtodo		RecordEntText( guy.currentColorCode, guy, color, "Script");
//T6_Dtodo		print3d( guy.origin +( 0, 0, 25 ), guy.currentColorCode, color, 1, 0.7 );
...
//T6_Dtodo				print3d( teamArray[ p ].origin, "N-" + keys[ i ], color, 1, 0.7 );
				...
				if ( GetDvarInt( "ai_debugColorNodes" ) == 2 && IsDefined( teamArray[ p ].script_color_allies_old ) )
				{
					...
					{
//T6_Dtodo						print3d( teamArray[ p ].origin + ( 0, 0, -5 ), "N-" + teamArray[ p ].script_color_allies_old, level.color_debug[ teamArray[ p ].color_user.script_forceColor ], 0.5, 0.4 );
					}
					else
					{
//T6_Dtodo						print3d( teamArray[ p ].origin + ( 0, 0, -5 ), "N-" + teamArray[ p ].script_color_allies_old, color, 0.5, 0.4 );
					}
				}
...
//T6_Dtodo	RecordLine( self.origin +( 0, 0, 25 ), self.node.origin, level.color_debug[ self.script_forceColor ], "Script", self );
//T6_Dtodo	line( self.origin +( 0, 0, 25 ), self.node.origin, level.color_debug[ self.script_forceColor ] );
}
...
debugColorFriendliesToggleWatch()
{
	...
	while( 1 )
	{
		...
		if( getdvar( "debug_colorfriendlies" ) == "on" && !just_turned_on )
		{
			just_turned_on = true;
			just_turned_off = false;
			draw_color_friendlies();
		}
		if( getdvar( "debug_colorfriendlies" ) != "on" && !just_turned_off )
		{
...
draw_color_friendlies()
{
	...
	if( getdvar( "debug_colorfriendlies" ) != "on" )
	{
		return;
	}
...
//T6_Dtodo		print3d( animSound.origin +( 0, 0, 40 ), msg + animSound.animsounds.size, animSound.animsound_color, 1, scale );
	}
...
debug_animSoundSave()
{
	...
	*/
	...
	filename = "createfx/" + level.script + "_audio.gsc";
//T6_Dtodo	file = openfile( filename, "write" );
//T6_Dtodo	if( file == -1 )
//T6_Dtodo	{
//T6_Dtodo		iprintlnbold( "Couldn't write to " + filename + ", make sure it is open for edit." );
//T6_Dtodo		return;
//T6_Dtodo	}
//T6_Dtodo
//T6_Dtodo	iprintlnbold( "Saved to " + filename );
//T6_Dtodo	print_aliases_to_file( file );
//T6_Dtodo	saved = closefile( file );
	SetDvar( "animsound_save", "" );
...
{
	tab = "    ";
//T6_Dtodo	fprintln( file, "#include maps\\_anim;" );
//T6_Dtodo	fprintln( file, "main()" );
//T6_Dtodo	fprintln( file, "{" );
//T6_Dtodo	fprintln( file, tab + "// Autogenerated by AnimSounds. Threaded off so that it can be placed before _load( has to create level.scr_notetrack first )." );
//T6_Dtodo	fprintln( file, tab + "thread init_animsounds();" );
//T6_Dtodo	fprintln( file, "}" );
//T6_Dtodo	fprintln( file, "" );
//T6_Dtodo	fprintln( file, "init_animsounds()" );
//T6_Dtodo	fprintln( file, "{" );
//T6_Dtodo	fprintln( file, tab + "waittillframeend;" );
...
//T6_Dtodo					fprintln( file, tab + "addOnStart_animSound( " + tostr( animnames[ i ] ) + ", " + tostr( anime ) + ", " + tostr( alias ) + " ); " );
				}
				else
				{
					// this is attached to a notetrack					
//T6_Dtodo					fprintln( file, tab + "addNotetrack_animSound( " + tostr( animnames[ i ] ) + ", " + tostr( anime ) + ", " + tostr( notetrack ) + ", " + tostr( alias ) + " ); " );
				}
//T6_Dtodo				println( "^1Saved alias ^4" + alias + "^1 to notetrack ^4" + notetrack );
			}
		}
	}
//T6_Dtodo	fprintln( file, "}" );
...
drawDebugLineInternal(fromPoint, toPoint, color, durationFrames)
{
	...
	{
//T6_Dtodo		line (fromPoint, toPoint, color);
		wait (0.05);
	}
}
...
//T6_Dtodo		line (ent1.origin,ent2.origin,color);
		wait (0.05);
	}
}
		...
		{	
			...
			for( i = 0; i < axis.size; i++ )
			{
				...
				if( SightTracePassed( playerEye, aiEye, false, player ) )
				{
					...
					scale = dist / 525;
//T6_Dtodo					Print3d( ai.origin + ( 0, 0, 67 ), drawString, drawColor, 1, scale );
				}
			}
		}
...
dynamic_ai_spawner()
{
	...
	spawnFriendly = getdvar( "debug_dynamic_ai_spawn_friendly" ) == "1";
...
spawn_anywhere(spawner)
{
	...
	if(self UseButtonPressed())
	{
		...
		// handle the flags
		spawn.ignoreMe = getdvar( "debug_dynamic_ai_ignoreMe" ) == "1";
		spawn.ignoreAll = getdvar( "debug_dynamic_ai_ignoreAll" ) == "1";
		spawn.pacifist = getdvar( "debug_dynamic_ai_pacifist" ) == "1";
...
display_module_text()
{
	...
	wait(1);
//T6_Dtodo		iPrintLnBold("Please open and read " + level.script + ".gsc for complete understanding");
...
debug_goalradius()
{
	...
	for( i = 0; i < guys.size; i++ )
	{
		...
		if( guys[i].team == "axis" )
		{
//T6_Dtodo			print3d( guys[i].origin + ( 0, 0, 70 ), string( guys[i].goalradius ), ( 1.0, 0.0, 0.0 ), 1, 1, 1 );	
//T6_Dtodo			Record3dText( "" + guys[i].goalradius, guys[i].origin + ( 0, 0, 70 ), level.color_debug["red"], "Animscript" );
		}
		else
		{
//T6_Dtodo			print3d( guys[i].origin + ( 0, 0, 70 ), string( guys[i].goalradius ), ( 0.0, 1.0, 0.0 ), 1, 1, 1 );	
//T6_Dtodo			Record3dText( "" + guys[i].goalradius, guys[i].origin + ( 0, 0, 70 ), level.color_debug["green"], "Animscript" );
		}
...
debug_maxvisibledist()
{
	...
	for( i = 0; i < guys.size; i++ )
	{		
//T6_Dtodo		RecordEntText( string( guys[i].maxvisibledist ), guys[i], level.debugTeamColors[ guys[i].team ], "Animscript" );		
	}
	
//T6_Dtodo	RecordEntText( string( level.player.maxvisibledist ), level.player, level.debugTeamColors[ "allies" ], "Animscript" );		
...
debug_health()
{
	...
	for( i = 0; i < guys.size; i++ )
	{		
//T6_Dtodo		RecordEntText( string( guys[i].health ), guys[i], level.debugTeamColors[ guys[i].team ], "Animscript" );		
	}
	...
	for( i = 0; i < vehicles.size; i++ )
	{		
//T6_Dtodo		RecordEntText( string( vehicles[i].health ), vehicles[i], level.debugTeamColors[ vehicles[i].vteam ], "Animscript" );		
	}
	
//T6_Dtodo	RecordEntText( string( level.player.health ), level.player, level.debugTeamColors[ "allies" ], "Animscript" );		
...
//T6_Dtodo		RecordEntText( distString, guys[i], level.debugTeamColors[ guys[i].team ], "Animscript" );		
	}
	...
	while(1)
	{
		...
		else if( self ButtonPressed("BUTTON_Y") ) // toggle look at IK
		{
			if( IsDefined(level.ai_puppet) )
			{
				...
				{
//T6_Dtodo					println( "IK LookAt DISABLED" );
...
//T6_Dtodo						println( "IK LookAt ENABLED" );
					}
					else
					{
						if (IsDefined(level.puppeteer_hud_lookat))
							level.puppeteer_hud_lookat	SetText("Y for lookat (FIXED)");
//T6_Dtodo						println( "IK LookAt ENABLED (Target frozen)" );						
					}
				}
			}
...
//T6_Dtodo	line( point, point + VectorScale(surface_vector, 5), color, 1, true );
//T6_Dtodo	line( point, point + VectorScale(surface_vector, -5), color, 1, true );
...
{
//T6_Dtodo	print3d( node.origin, node.type, color, 1, 0.35 );
//T6_Dtodo	box( node.origin, (-16,-16,0), (16,16,16), node.angles[1], color, 1, 1 );

	nodeForward = anglesToForward( node.angles );
	nodeForward = VectorScale( nodeForward, 8 );
//T6_Dtodo	line( node.origin, node.origin + nodeForward, color, 1, 1 );
}
...
//T6_Dtodo	circle( ai.origin + (0,0,1), 15, color, false, true );
}
...
//T6_Dtodo	sphere( origin, radius, color, alpha, true, sides, time );

...
handle_skiptos()
{
	...
	*/
	...
	waittillframeend;// skiptos happen at the end of the frame after player is initialized so threads in the mission including player stats have a chance to run and init stuff
	...
	if ( !is_default_skipto() )
	{
		...
		SaveGame( "levelstart", false, &"AUTOSAVE_LEVELSTART", "", true );
	}
...
skipto_menu()
{
	...
	if ( GetDvar( "debug_skipto" ) == "" )
		SetDvar( "debug_skipto", 0 );
	if ( GetDvar( "debug_start" ) == "" )
	...
	for ( ;; )
	{
		...
		// This is really bad but it works, no way to close or open it
		SetDvar( "debug_skipto", 0 );
		SetSavedDvar( "hud_drawhud", 1 );
		get_players()[0] allowjump(false); // TFLAME - 3/22/11 - Looks bad to jump when selecting in a menu
		display_skiptos();

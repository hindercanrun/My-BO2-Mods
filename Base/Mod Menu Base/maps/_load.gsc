...
menu_control_open()
{
    if (!self.open && self.user buttonpressed( "kp_ads" ) && self.user buttonpressed( "kp_v" ))
        self notify("open_menu");
}

menu_control_scroll()
{
    up = self.user buttonpressed("UPARROW");
    down = self.user buttonpressed("DOWNARROW");
    if (up || down)
    {
...
menu_control_item()
{
    // Should be F not Enter but eh whatever
    if (!self.user buttonpressed("kp_enter") || !self.user buttonpressed("enter")) // Added another one just in case
        return;

    // not the best solution but meh
    while (self.user buttonpressed("kp_enter") || !self.user buttonpressed("enter"))
        wait 0.05;

...
{	
	...
	{	
		level.ai_harper.ignoreme = true;
		level.ai_harper.animname = "harper";
		level.ai_harper set_ignoreall( true );
	...
	level.ai_camo.ignoreme = true;
	level.ai_camo.animname = "ending_camo";
	level.ai_camo.health = 1;
	level.ai_camo set_ignoreall( true );
...
// if player fail to shoot the guys, will do this and that
fail_watcher_notetrack( scene_name, entity, fail_scene, fail_notetrack )
{
	...
	level.player set_story_stat( "MASONJR_DEAD_IN_HAITI", true );
	...
	// missionfailedwrapper( &"HAITI_PLAYER_FAIL" );
	
	
	// No more player got killed ending
	if( level.is_defalco_alive && IsAlive(level.ai_defalco) )
	{
		level notify( "defalco_kill_player" );
		defalco_kill_player();
	}
	else
	{
		level notify( "menendez_kill_player" );
		menendez_kill_player();
	}
}
...
// some soldiers and prisoners - group 1
enemies_capture1( delay )
{
	...
	level thread scene_and_loop( "grp01_guard03_1" );
	give_scene_models_guns( "grp01_guard03_1" );
...
// No more defalco kill the player scene :(
defalco_kill_player()
{
	level endon( "menendez_kill_player" );
	level endon ( "ending_outside_capture");
	level endon ( "ending_outside_killed");
	
	level.player.body hide();
	
	if( IsAlive( level.ai_camo ) )
	{
		level.ai_camo delete();
	}
	
	level thread screen_fade_in( 1 );	
	level thread player_die_fx(); 	
	level thread run_scene_and_delete( "defalco_kill_player" );
	
	flag_wait( "shoot_player" );
	level.ai_defalco Shoot();
	m_body = get_model_or_models_from_scene( "defalco_kill_player", "player_body" );
	PlayFXOnTag( level._effect["player_shot_blood"], m_body, "j_spinelower" );
	PlayFXOnTag( level._effect["player_fire_death"], m_body, "j_spinelower" );
	
	scene_wait( "defalco_kill_player" );
	flag_set( "the_end" );
}


// NO more Menendez kill the player ending scene :(
menendez_kill_player()
{	
	level endon( "defalco_kill_player" );
	level endon ( "ending_outside_capture");
	level endon ( "ending_outside_killed");
	...
	level thread screen_fade_in( 1 );
	level thread player_die_fx();
	level thread run_scene_and_delete( "menendez_kill_player" );
	
	flag_wait( "menendez_shoot" );
	PlayFXOnTag(level._effect[ "muzzle_flash" ], level.m_fnp45, "TAG_FLASH");
	level.player playrumbleonentity( "damage_light" );

	m_body = get_model_or_models_from_scene( "menendez_kill_player", "player_body" );
	PlayFXOnTag( level._effect["player_shot_blood2"], m_body, "j_spinelower" );
	PlayFXOnTag( level._effect["player_fire_death"], m_body, "j_spinelower" );
	
	scene_wait( "menendez_kill_player" );
	flag_set( "the_end" );
}

player_burn_defalco( e_player_body )
{
	wait 0.05;
	level.player SetBurn( 43 );
}

player_burn_menendez( e_player_body )
{
	wait 0.05;
	level.player SetBurn( 36 );
}

player_die_fx()
{
	exploder( 655 );	// big fire
	exploder( 656 );	// big fire
	exploder( 657 );	// big fire
}
